...
function saveApprovedGroups(groups) {
  fs.writeFileSync(APPROVED_FILE, JSON.stringify(groups, null, 2));
}

let approvedGroups = loadApprovedGroups();
...
function isPremium(userId) {
  return isOwner(userId) || (config.PREMIUM_USERS || []).includes(String(userId));
}

function canUseGroupFeature(chat) {
  if (!isGroupChat(chat)) return false;
  return approvedGroups.includes(String(chat.id));
}

function isApprovedGroup(chatId) {
  return approvedGroups.includes(String(chatId));
}

// contoh: untuk fitur tertentu yang hanya boleh dipakai setelah grup di-ACC
function canCreatePanel(userId, chat) {
  // wajib premium (atau owner)
  if (!isPremium(userId)) return false;

  // kalau dipakai di grup: grup harus sudah di-ACC
  if (isGroupChat(chat)) return isApprovedGroup(chat.id);

  // kalau dipakai di private chat, bebas (atau kamu bisa atur juga)
  return true;
}
...
// - hanya di grup
// - hanya owner
// - menambahkan chat.id ke approvedGroups
bot.onText(/^\/accgc(?:\s+(.+))?$/i, (msg) => {
  const chatId = msg.chat.id;
  const senderId = msg.from.id;

  if (!isGroupChat(msg.chat)) {
    return bot.sendMessage(chatId, "❌ /accgc hanya bisa digunakan di dalam grup.");
  }

  if (!isOwner(senderId)) {
    return bot.sendMessage(chatId, "❌ Akses ditolak, hanya Owner yang bisa pakai /accgc.");
  }

  const idStr = String(chatId);
  if (approvedGroups.includes(idStr)) {
    return bot.sendMessage(chatId, "✅ Grup ini sudah di-ACC sebelumnya.");
  }

  approvedGroups.push(idStr);
  saveApprovedGroups(approvedGroups);

  return bot.sendMessage(chatId, "✅ Berhasil! Grup ini sekarang sudah di-ACC, fitur reseller group sudah aktif.");
});

// ===== Command: /delaccgc =====
// - hanya di grup
// - hanya owner
// - menghapus grup dari approved list
bot.onText(/^\/delaccgc$/i, (msg) => {
  const chatId = msg.chat.id;
  const senderId = msg.from.id;

  if (!isGroupChat(msg.chat)) {
    return bot.sendMessage(chatId, "❌ /delaccgc hanya bisa digunakan di dalam grup.");
  }

  if (!isOwner(senderId)) {
    return bot.sendMessage(chatId, "❌ Akses ditolak, hanya Owner yang bisa pakai /delaccgc.");
  }

  const idStr = String(chatId);

  if (!approvedGroups.includes(idStr)) {
    return bot.sendMessage(chatId, "⚠️ Grup ini belum di-ACC.");
  }

  approvedGroups = approvedGroups.filter(id => id !== idStr);
  saveApprovedGroups(approvedGroups);

  return bot.sendMessage(chatId, "❌ ACC grup berhasil dicabut. Fitur reseller grup telah dinonaktifkan.");
});
...
bot.onText(/\/1gb (.+)/, async (msg, match) => {
  ...
  if (!isGroupChat(msg.chat)) {
    return bot.sendMessage(chatId, "❌ Perintah ini hanya bisa digunakan di dalam grup reseller.");
  }

  if (!canUseGroupFeature(msg.chat)) {
    return bot.sendMessage(
      chatId,
      "❌ Akses ditolak.\nGrup ini belum di-ACC.\nOwner bot silakan ketik /accgc"
    );
  }
